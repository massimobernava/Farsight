 ...
 * device_id: identifies this machine in the dashboard/Grafana — keep it
 *   stable, it's also the InfluxDB tag used for history. Which tenant
 *   (customer/group) this device belongs to is decided entirely
 *   server-side by an admin, not here — a new device_id just shows up
 *   under the "default" tenant until someone moves it.
 *
 * Returns NULL on failure (connection refused, invalid broker_url, ...). */
farsight_client *farsight_connect(const char *broker_url,
                                   const char *device_id);

/* Same as farsight_connect, but reads broker_url/device_id from a
 * farsight client.conf file (MQTT_BROKER, DEVICE_ID — same format/keys
 * as /etc/farsight/client.conf, since this is meant to run alongside a
 * real farsight-agent install and share its identity). Pass NULL for
 * config_path to use /etc/farsight/client.conf (or the
 ...
 *
 * Returns NULL on failure (file missing/unreadable, MQTT_BROKER missing
 * from it, or the connection itself failing). */
 ...
 * docs/DEVELOPMENT.md "Upload file client→server"). Talks to
 * farsight-server's HTTP control plane, not the MQTT broker — same host as
 * broker_url (that's always true in a real deployment: one farsight-server
 * package installs both), http_port is that host's HTTP_PORT from
 * server.conf (pass 0 for the default, 8080).
 *
