...
mkdir -p "$ROOT/DEBIAN" \
         ...
         "$ROOT/usr/share/farsight/grafana" \
         "$ROOT/usr/share/farsight/grafana-plugins/farsight-llmassistant-app" \
...
echo "==> building farsight-grafana-proxy ($ARCH)"
( cd "$REPO_ROOT" && \
  CGO_ENABLED=0 GOOS=linux GOARCH="$ARCH" go build -o "$ROOT/usr/bin/farsight-grafana-proxy" ./cmd/farsight-grafana-proxy )

echo "==> building farsight-llmassistant-app Grafana plugin (frontend-only, no backend to cross-compile)"
( cd "$REPO_ROOT/grafana-plugin/llm-assistant-app" && \
  npm install --no-audit --no-fund && \
  npm run build )
cp -r "$REPO_ROOT/grafana-plugin/llm-assistant-app/dist/." "$ROOT/usr/share/farsight/grafana-plugins/farsight-llmassistant-app/"
...
install -m 0644 "$PKG_SRC/lib/systemd/system/farsight-grafana-proxy.service" "$ROOT/lib/systemd/system/"
...
install -m 0644 "$PKG_SRC/etc/farsight/grafana/sistema.json.tmpl" "$ROOT/usr/share/farsight/grafana/sistema.json.tmpl"
